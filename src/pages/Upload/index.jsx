...
function Upload() {
  ...
  // 🔥 Firestore থেকে ডাইনামিকালি কোর্স ফেচ করার Effect Hook
  useEffect(() => {
    const fetchCoursesFromFirestore = async () => {
      // Dept এবং Semester ২টাই সিলেক্ট না থাকলে কোর্স ফেচ করার দরকার নেই
      ...
      try {
        // Firestore-এ departmentId এবং semesterId দিয়ে Query করা
        const q = query(
          collection(db, "courses"),
          where("departmentId", "==", formData.department.toLowerCase()),
          where("semesterId", "==", formData.semester.toLowerCase())
        );
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation Checks
    if (!title.trim()) return setError("Resource title is required.");
    if (!department) return setError("Please select a department.");
    if (!semester) return setError("Please select a semester.");
    if (!course) return setError("Course selection is required.");
    if (!type) return setError("Please select a resource type.");
    if (!description.trim()) return setError("Description is required.");
    if (!driveLink.trim()) return setError("Please enter Google Drive link.");
    ...
    <section className="mx-auto max-w-4xl px-6 py-16">
      <div className="rounded-3xl border border-slate-800 bg-slate-900 p-10 shadow-2xl">
        <div className="mb-10 text-center">
          <UploadCloud size={55} className="mx-auto text-blue-400" />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Title */}
          <div>
            <label className="mb-2 block text-slate-300">Resource Title</label>
            <input
              type="text"
              name="title"
          ...
          {/* Semester */}
          <div>
            <label className="mb-2 block text-slate-300">Semester</label>
            <select
              name="semester"
            ...
            >
              ...
              {Array.from({ length: 12 }, (_, index) => (
                <option key={index + 1} value={`semester-${index + 1}`}>
                  Semester {index + 1}
                </option>
              ))}
            </select>
          ...
          {/* Dynamic Course Dropdown from Firestore */}
          <div>
            <label className="mb-2 block text-slate-300">Course</label>
            <select
              name="course"
              value={formData.course}
              onChange={handleChange}
              disabled={!formData.department || !formData.semester || isLoadingCourses}
          ...
          {/* Resource Type */}
<div>
  <label className="mb-2 block text-slate-300">Resource Type</label>
  <select
    name="type"
    value={formData.type}
    onChange={handleChange}
  ...
  >
    ...
    <option value="Lectures">Lectures</option> {/* Assignment এর জায়গায় Lectures যোগ করা হলো */}
